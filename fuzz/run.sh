...
log="$here/log.txt"
corpus="$here/corpus/pretty_diff"
...
# distinct corner of the pretty printer: major types, tags, floats, indefinite
...
cd "$here"
echo "== differential fuzz: pretty printer vs upstream C oracle, ${DURATION}s ==" | tee "$log"
status=0
cargo +nightly fuzz run pretty_diff -- \
  "-max_total_time=$DURATION" -print_final_stats=1 "${extra[@]}" 2>&1 | tee -a "$log" || status=$?

if [ "$status" -ne 0 ]; then
  echo "== DIVERGENCE OR CRASH -- see $log and fuzz/artifacts/pretty_diff/ ==" | tee -a "$log"
fi
exit "$status"
